...
export default function PaymentPage({ cart }: { cart: OdooCart | undefined }) {
  /* eslint-disable */
  const [state, formAction] = useFormState(createPaymentMethod, null);
            ...
            <tr className="border-b dark:border-gray-700">
              ...
              >
                ...
                <Price
                  ...
                  amount={shippingAddress?.selected_shipping_method?.amount?.value || 0}
                  currencyCode={
                    shippingAddress?.selected_shipping_method?.amount?.currency || 'USD'
                  }
                  ...
                  {isArray(payments) &&
                    payments?.map((pay) => (
                      <Radio
                        key={pay.title}
                        value={pay}
                        className="group relative flex cursor-pointer rounded-lg bg-slate-100 px-5 py-7 text-white shadow-md transition focus:outline-none data-[checked]:bg-slate-200 data-[focus]:outline-1 data-[focus]:outline-white dark:bg-white/5 data-[checked]:dark:bg-white/10"
                      >
