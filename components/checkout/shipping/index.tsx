...
export default function ShippingMethod({ cartData }: { cartData: OdooCart | undefined }) {
  const availableShippingMethods = cartData?.shipping_address?.available_shipping_methods || [];
  /* eslint-disable */
  const [state, formAction] = useFormState(createShippingMethod, null);
              ...
              >
                {availableShippingMethods.map((method) => (
                  <Radio
                    ...
                    value={method}
                    className="group relative flex cursor-pointer rounded-lg bg-slate-100 px-5 py-4 text-white shadow-md transition focus:outline-none data-[checked]:bg-slate-200 data-[focus]:outline-1 data-[focus]:outline-white dark:bg-white/5 data-[checked]:dark:bg-white/10"
                  >
