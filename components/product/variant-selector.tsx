...
export function VariantSelector({
  ...
  const hasNoOptionsOrJustOneOption =
    !isArray(options) || (options.length === 1 && options[0]?.values.length === 1);
  ...
  const combinations: Combination[] = variants.map((variant) => ({
    ...
    ...variant?.attributes?.reduce(
      (accumulator, option) => ({ ...accumulator, [option.code.toLowerCase()]: option.label }),
      {}
    )
  }));
    ...
    <dl className="mb-8" key={option.uid}>
      ...
      <dd className="flex flex-wrap gap-3">
        {option.values.map((value) => {
          ...
          const isActive = searchParams.get(optionNameLowerCase) === value.label;
